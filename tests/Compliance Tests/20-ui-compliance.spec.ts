...
test.describe('MGA/IOM UI Compliance', () => {
  ...
  test('TLIB-27: Clock (HH:MM) visible during gameplay', async ({ gamePage }) => {
  ...
  test('TLIB-79: Game texts in applicable language (English)', async ({ gamePage }) => {
  ...
  test('TLIB-87: Balance displayed at all times (before spin)', async ({ gamePage }) => {
  ...
  test('TLIB-87: Balance remains visible after spin', async ({ gamePage }) => {
    await gamePage.spinAndWait();
    await expect(gamePage.balanceDisplay).toBeVisible({ timeout: 5_000 });
  });

  test('TLIB-178: Currency denomination clear to the player', async ({ gamePage }) => {
    const page = gamePage.page;
  ...
  test('TLIB-179: Balance, total bet, and denomination all displayed', async ({ gamePage }) => {
  ...
  test('TLIB-23: Free-to-play (demo) mode uses same UI as real mode', async ({ gamePage, gameUrl }) => {
